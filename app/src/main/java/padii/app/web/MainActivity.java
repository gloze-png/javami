...
public class MainActivity extends AppCompatActivity {
    ...
    static boolean PadiiApp_JSCRIPT = PadiiConfig.PadiiApp_JSCRIPT;
    static boolean PadiiApp_FUPLOAD = PadiiConfig.PadiiApp_FUPLOAD;
    static boolean PadiiApp_CAMUPLOAD = PadiiConfig.PadiiApp_CAMUPLOAD;
    static boolean PadiiApp_ONLYCAM = PadiiConfig.PadiiApp_ONLYCAM;
    static boolean PadiiApp_MULFILE = PadiiConfig.PadiiApp_MULFILE;
    static boolean PadiiApp_LOCATION = PadiiConfig.PadiiApp_LOCATION;
    //static boolean SngineApp_RATINGS = PadiiConfig.PadiiApp_RATINGS;
    static boolean PadiiApp_PULLFRESH = PadiiConfig.PadiiApp_PULLFRESH;
    static boolean PadiiApp_PBAR = PadiiConfig.PadiiApp_PBAR;
    static boolean PadiiApp_ZOOM = PadiiConfig.PadiiApp_ZOOM;
    static boolean PadiiApp_SFORM = PadiiConfig.PadiiApp_SFORM;
    static boolean PadiiApp_OFFLINE = PadiiConfig.PadiiApp_OFFLINE;
    static boolean PadiiApp_EXTURL =PadiiConfig.PadiiApp_EXTURL;

    //Security variables
    static boolean PadiiApp_CERT_VERIFICATION = PadiiConfig.PadiiApp_CERT_VERIFICATION;

    //Configuration variables
    private static String Padii_URL = PadiiConfig.Padii_URL;
    private String CURR_URL = Padii_URL;
    private static String Padii_F_TYPE = PadiiConfig.Padii_F_TYPE;

    public static String ASWV_HOST = aswm_host(Padii_URL);
...
//        }
        ...
        swvp_view.getSettings().setUserAgentString("Padii");
        swvp_view.getSettings().setMediaPlaybackRequiresUserGesture(false);


        if (!PadiiApp_OFFLINE) {
            webSettings.setJavaScriptEnabled(PadiiApp_JSCRIPT);
        }
        webSettings.setSaveFormData(PadiiApp_SFORM);
        webSettings.setSupportZoom(PadiiApp_ZOOM);
        webSettings.setGeolocationEnabled(PadiiApp_LOCATION);
                ...
                if (check_permission(3)) {
                    if (PadiiApp_FUPLOAD) {
                        ...
                        if (!PadiiApp_ONLYCAM) {
                            ...
                            contentSelectionIntent.setType(Padii_F_TYPE);
                            if (PadiiApp_MULFILE) {
    ...
    public boolean url_actions(WebView view, String url) {
        ...
        if (!PadiiApp_OFFLINE && !DetectConnection.isInternetAvailable(MainActivity.this)) {
...
//            }
            ...
            String offloc = Padii_URL + "?loc=" + get_location();
            aswm_view(offloc, false);
            Log.d("OFFLINE LOC REQ", offloc);

            //Opening external URLs in android default web browser
        } else if (PadiiApp_EXTURL && !aswm_host(url).equals(ASWV_HOST)) {
            aswm_view(url, true);
        } else {
            a = false;
        }
    ...
    public void pull_fresh() {
        aswm_view((!CURR_URL.equals("") ? CURR_URL : Padii_URL), false);
    }
    ...
    public void get_info() {
        ...
        cookieManager.setCookie(Padii_URL, "DEVICE=android");
        cookieManager.setCookie(Padii_URL, "DEV_API=" + Build.VERSION.SDK_INT);
    }
    ...
    public void get_file() {
        ...
        if (PadiiApp_FUPLOAD && PadiiApp_CAMUPLOAD && !check_permission(2) && !check_permission(3)) {
            ActivityCompat.requestPermissions(MainActivity.this, perms, file_perm);

            //Checking for WRITE_EXTERNAL_STORAGE permission
        } else if (PadiiApp_FUPLOAD && !check_permission(2)) {
            ActivityCompat.requestPermissions(MainActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE}, file_perm);

            //Checking for CAMERA permissions
        } else if (PadiiApp_CAMUPLOAD && !check_permission(3)) {
    ...
    public String get_location() {
        ...
        if (PadiiApp_LOCATION && (Build.VERSION.SDK_INT < 23 || check_permission(1))) {
            GPSTrack gps;
            ...
            if (gps.canGetLocation()) {
                if (latitude != 0 || longitude != 0) {
                    if (!PadiiApp_OFFLINE) {
                        CookieManager cookieManager = CookieManager.getInstance();
                        cookieManager.setAcceptCookie(true);
                        cookieManager.setCookie(Padii_URL, "lat=" + latitude);
                        cookieManager.setCookie(Padii_URL, "long=" + longitude);
                    }
      ...
      NotificationCompat.Builder builder = new NotificationCompat.Builder(MainActivity.this, "");
       switch (type) {
           case 1:
                ...
                builder.setSmallIcon(R.drawable.pal);
               break;
//
//            case 2:
//                builder.setTicker(getString(R.string.app_name));
...
//                builder.setSmallIcon(R.drawable.padii);
//                break;
       }
